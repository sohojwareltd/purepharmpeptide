...
@section('content')
    <div class="container py-5">
        ...
        <div class="section-header text-center mb-5">
            <h1 class="section-title display-4 fw-bold"
                style="font-family: 'Playfair Display', serif; color: var(--primary); letter-spacing: 1px;">
                <i class="bi bi-cart3 me-3"></i>Shopping Cart
            </h1>
            <p class="section-subtitle lead" style="font-family: 'Inter', sans-serif; color: var(--text-muted);">Review your
                items and proceed to checkout</p>
        </div>
        ...
        @if (!empty($cartItems) && count($cartItems) > 0)
            <div class="row">
                ...
                <div class="col-lg-8 mb-4">
                    <div class="card shadow-lg border-0 premium-cart-card p-4">
                        <div class="card-header bg-white border-bottom-0 pb-0">
                            <h5 class="mb-0 fw-bold" style="font-family: 'Playfair Display', serif; color: var(--primary);">
                        ...
                        <div class="card-body p-0">
                                            ...
                                            <tr class="cart-item-table-row" data-item-id="{{ $itemKey }}">
                                                ...
                                                <td class="d-flex align-items-center gap-3 py-3">
                                                    ...
                                                    <div>
                                                        <div class="fw-semibold"
                                                            style="font-family: 'Playfair Display', serif; font-size: 1rem; color: var(--primary);">
                                                            {{ $item['product_name'] }}</div>
                                                        @if (isset($item['pricing_type']) && $item['pricing_type'])
                                                            <small class="text-muted">
                                                                <i class="bi bi-tag"></i>
                                                                {{ ucfirst($item['pricing_type']) }} Pricing
                                                            </small>
                                                        @endif
                                                    </div>
                                                </td>
                                                ...
                                                <td class="text-center">
                                                    <div class="d-flex align-items-center justify-content-center gap-2">
                                                        <button class="btn btn-outline-secondary btn-sm quantity-btn px-2"
                                                            data-action="decrease" data-item-id="{{ $itemKey }}"><i
                                                                class="bi bi-dash"></i></button>
                                                        <input type="number"
                                                            class="form-control form-control-sm text-center mx-1 quantity-input"
                                                            value="{{ $item['quantity'] }}" min="1"
                                                            max="{{ $item['product']['stock'] ?? 999 }}"
                                                            data-item-id="{{ $itemKey }}" style="width: 48px;">
                                                        <button class="btn btn-outline-secondary btn-sm quantity-btn px-2"
                                                            data-action="increase" data-item-id="{{ $itemKey }}"><i
                                                ...
                                                <td class="text-end">
                                                    <button class="btn btn-link text-danger p-0 remove-item"
                                                        data-item-id="{{ $itemKey }}" title="Remove item">
                            ...
                            <div class="d-block d-md-none">
                                @foreach ($cartItems as $itemKey => $item)
                                    <div class="cart-mobile-card mb-3 p-3 shadow-sm rounded-4 position-relative"
                                        ...
                                        <div class="d-flex align-items-center gap-3 mb-2">
                                            ...
                                            <div class="flex-grow-1">
                                                <div class="fw-semibold mb-1"
                                                    style="font-family: 'Playfair Display', serif; color: var(--primary);">
                                                    {{ $item['product_name'] }}</div>
                                                @if (isset($item['pricing_type']) && $item['pricing_type'])
                                                    <small class="text-muted">
                                                        <i class="bi bi-tag"></i> {{ ucfirst($item['pricing_type']) }}
                                                        Pricing
                                                    </small>
                                                @endif
                                            </div>
                                            <button
                                                class="btn btn-link text-danger p-0 remove-item position-absolute top-0 end-0 mt-2 me-2"
                                                data-item-id="{{ $itemKey }}" title="Remove item">
                                                <i class="bi bi-trash fs-5"></i>
                                            </button>
                                        </div>
                                        <div class="d-flex align-items-center justify-content-between mt-2">
                                            ...
                                            <div class="text-end">
                                                ...
                                                <div class="fw-bold small" style="color: var(--primary);">Total:
                                                    ${{ number_format($item['total'], 2) }}</div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <!-- Cart Summary -->
                <div class="col-lg-4">
                    <div class="card shadow-lg border-0 sticky-top premium-summary-card"
                        style="top: 2rem; background: linear-gradient(135deg, #fff 80%, var(--light) 100%); border-radius: 1.5rem;">
                        <div class="card-header bg-white border-bottom-0 pb-0">
                            <h5 class="mb-0 fw-bold"
                                style="font-family: 'Playfair Display', serif; color: var(--success-color);">
                            ...
                            <div class="summary-details premium-summary-details">
                                ...
                                <div class="d-flex justify-content-between mb-4">
                                    <span class="fw-bold fs-5"
                                        style="font-family: 'Playfair Display', serif;">Total:</span>
                                    <span class="fw-bold fs-4 price premium-price"
                                        style="font-family: 'Playfair Display', serif; color: var(--primary);">${{ number_format($total, 2) }}</span>
                                </div>
                            </div>
            ...
            <div class="text-center py-5">
                <div class="card shadow-lg border-0" style="max-width: 500px; margin: 0 auto;">
                    <div class="card-body p-5">
                        ...
                        <h3 class="fw-bold mb-3" style="font-family: 'Playfair Display', serif; color: var(--primary);">
    ...
    <style>
        .premium-cart-card {
            ...
            background: linear-gradient(135deg, #fff 80%, var(--light) 100%);
        ...
        <script>
            $(document).ready(function() {
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                if (response.cart_items && response.cart_items[itemId]) {
                                    ...
                                    itemElement.find('.fw-bold').text('$' + parseFloat(item.total).toFixed(
                                        2));
                    ...
                    if (confirm('Are you sure you want to remove this item?')) {
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    updateCartCount();
                                    $(`.cart-item-table-row[data-item-id="${itemId}"]`).fadeOut(300,
                                        function() {
                                            $(this).remove();
                ...
                <div class="d-flex justify-content-between mb-4">
                    ...
                    <span class="fw-bold fs-4 price premium-price" style="font-family: 'Playfair Display', serif; color: var(--primary);">$${parseFloat(data.total || data.cart_total || 0).toFixed(2)}</span>
                </div>
            `);
                }
            });
